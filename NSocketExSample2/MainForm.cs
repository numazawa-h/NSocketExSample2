...
namespace SampleMain
{
    ...
    {
        SampleSocket listensocket = new SampleSocket();
        ...
        {
            SampleSocket connectsocket = new SampleSocket();
            connectsocket.OnExceptionEvent += OnException;
            connectsocket.OnDisConnectEvent += OnDisConnect;
            connectsocket.OnConnectEvent += OnConnect;
            ...
            if (iaddr1.Length > 0)
            {
                try
                {
                    connectsocket.SetSelfEndPoint(iaddr1, portno1);
                }
                catch (Exception ex)
                {
            ...
            connectsocket.Connect(iaddr2, portno2);
        }
        ...
        private void OnDisConnect(object sender, NSocketEventArgs args)
        {
            ...
            SampleSocket socket = (SampleSocket)args.Socket;
            DisplayLog($"OnDisConnect {socket.RemoteIPAddress}:{socket.RemotePortno}");
        }
